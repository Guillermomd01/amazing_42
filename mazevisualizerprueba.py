...
class MazeVisualizer():
    ...
    def render(self):
        ...
        self._fill_cell(self.maze.entry[0], self.maze.entry[1], self.start_color)
        self._fill_cell(self.maze.exit[0], self.maze.exit[1], self.exit_color)
        ...
        for y in range(self.maze.height):
            for x in range(self.maze.width):
                ...
                if val & 1: # N
                    for i in range(self.tile_size): self._put_pixel(px + i, py, self.wall_color)
                if val & 2: # E
                    for i in range(self.tile_size): self._put_pixel(px + self.tile_size - 1, py + i, self.wall_color)
                if val & 4: # S
                    for i in range(self.tile_size): self._put_pixel(px + i, py + self.tile_size - 1, self.wall_color)
                if val & 8: # W
                    for i in range(self.tile_size): self._put_pixel(px, py + i, self.wall_color)
