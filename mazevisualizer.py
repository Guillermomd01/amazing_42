...
class MazeVisualizer():
    def __init__(self, maze, tile_size=30):
        ...
        if not self.mlx_ptr:
            print("Error: No se pudo conectar con el servidor gráfico (MLX_INIT falló)")
            sys.exit(1)
    ...
    def _draw_line(self, x1, y1, x2, y2, color):
        ...
        if x1 == x2: # Línea vertical
            for y in range(min(y1, y2), max(y1, y2)):
                self.mlx.mlx_pixel_put(self.mlx_ptr, self.win_ptr, x1, y, color)
        elif y1 == y2: # Línea horizontal
            for x in range(min(x1, x2), max(x1, x2)):
                self.mlx.mlx_pixel_put(self.mlx_ptr, self.win_ptr, x, y1, color)
    ...
    def close_window(self, keycode, param=None):
        if keycode == 53 or keycode == 65307:
            sys.exit(0)
        return 0

    def run(self):
        self.mlx.mlx_key_hook(self.win_ptr, self.close_window, None)
        self.render()
